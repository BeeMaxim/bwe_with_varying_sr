...
class InnerAero(nn.Module):
    ...
    def forward(self, x, initital_sr, target_sr, return_spec=False, return_lr_spec=False, **batch):
        length = x.shape[-1]
        ...
        import torchaudio
        #print(x.shape)
        #torchaudio.save(f'D:\hifi_ssr\\bwe_with_varying_sr\demos\\final.wav', x[0].cpu().detach(), target_sr)
...
class Aero(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.generator = InnerAero(*args, **kwargs)
